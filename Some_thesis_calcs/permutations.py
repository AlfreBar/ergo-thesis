...
def build_gdict(final_dict, perm_dict,lam,gamma=1):
    ...
    for strings in final_dict:
        ...
        for j,perm in enumerate(final_dict[strings]):
            ...
            for i in range(0,len(perm),2):
                ...
                gval=gval*assign_g(int(perm[i]),int(perm[i+1]),lam,gamma)
...
def rho_2new(final_dict,perm_dict,l,gamma=1):

    start=time.time()
    gd=build_gdict(final_dict, perm_dict,l,gamma)
    mid=time.time()
    rho=0
    for s in gd:
        if len(s)==2:
