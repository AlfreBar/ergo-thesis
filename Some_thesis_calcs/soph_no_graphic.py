...
import sys
try:
    from collections.abc import namedtuple, Callable  # noqa
except ImportError:
    from collections import namedtuple, Callable  # noqa
...
class XYmodel(object):
    ...
    def H1(self, site_index):
        half_hz = -0.5 * self.hz(site_index)
        half_hx = -0.5 * self.hx(site_index)
        return np.array([[half_hz, half_hx], [half_hx, -half_hz]], self.dtype)
    ...
    def enlarge_block(self, block, direction, bare_site_index):
        ...
        if self.boundary_condition == open_bc:
            enlarged_operator_dict = {
                "H": kron(o["H"], identity(self.d)) +
                     kron(identity(mblock), self.H1(bare_site_index)) +
                     self.H2(o["conn_Sx"],o["conn_Sy"], self.Sx1, self.Sy1),
                "conn_Sx": kron(identity(mblock), self.Sx1),
                "conn_Sy": kron(identity(mblock), self.Sy1)
            }
        else:
            assert direction in ("l", "r")
            if direction == "l":
    ...
    def enlarge_block(self, block, direction, bare_site_index):
        ...
        if self.boundary_condition == open_bc:
            enlarged_operator_dict = {
                "H": kron(o["H"], identity(self.d)) +
                     kron(identity(mblock), self.H1(bare_site_index)) +
                     self.H2(o["conn_Sx"],o["conn_Sy"], self.Sz1, self.Sy1),
                "conn_Sx": kron(identity(mblock), self.Sz1),
                "conn_Sy": kron(identity(mblock), self.Sy1)
            }
        else:
            assert direction in ("l", "r")
            if direction == "l":
                enlarged_operator_dict = {
                    "H": kron(o["H"], identity(self.d)) +
                         kron(identity(mblock), self.H1(bare_site_index)) +
                         self.H2(o["l_Sx"], o["l_Sy"], self.Sz1, self.Sy1),
                    "l_Sx": kron(identity(mblock), self.Sz1),
                    "l_Sy": kron(identity(mblock), self.Sy1),
                    "r_Sx": kron(o["r_Sx"], identity(self.d)),
                    "r_Sy": kron(o["r_Sy"], identity(self.d)),
                }
                ...
                enlarged_operator_dict = {
                    "H": kron(o["H"], identity(self.d)) +
                         kron(identity(mblock), self.H1(bare_site_index)) +
                         self.H2(o["r_Sx"], o["r_Sy"], self.Sz1, self.Sy1),
                    "l_Sx": kron(o["l_Sx"], identity(self.d)),
                    "l_Sy": kron(o["l_Sy"], identity(self.d)),
                    "r_Sx": kron(identity(mblock), self.Sz1),
                    "r_Sy": kron(identity(mblock), self.Sy1),
                }
...
def perform_dmrg(model, L, m_warmup, m_sweep_list, target_sector=None, measurements=None):
    ...
    # # print("=", "{:.20f}".format(ev))
    return returned_measurements,energy,psi0

if __name__ == "__main__":
    np.set_printoptions(precision=10, suppress=True, threshold=10000, linewidth=300)
#%%    

    def run_sample_spin_chain(boundary_condition, L=40):
        model = XYmodel(J=1., gamma=1, hz=0.5, boundary_condition=boundary_condition)
        #measurements = ([[(i, "Sx")] for i in range(L)])

        return  perform_dmrg(model, L=L, m_warmup=5, m_sweep_list=[5,5,5,10,10, 40], target_sector=None, measurements=None)

    start=time.time()
    a,b,c=run_sample_spin_chain(open_bc)
    print(time.time()-start)
    import matplotlib.pyplot as plt
    measures={}
    lambdarange=np.linspace(0.8,1.2,4)
    for gs in [0.5,1,1.5]:
        lastm=40
        for l,accaz in enumerate(lambdarange):
            start=time.time()
            print(l)
            L=40
            model = XYmodel(J=1., gamma=gs,hz=accaz, hx=0., boundary_condition=open_bc)
            #spinz = ([[(i, "Sz")] for i in range(L)])
            #spincorr=([[(j, "Sz"), (j+3, "Sz")] for j in range(L-3)] )
            measures[l,gs]=perform_dmrg(model, L=L, m_warmup=5, m_sweep_list=[5,5,5,10,10,40], target_sector=None, measurements=None)
            end=time.time()
            print(end-start)
...
#%%
    ...
    for gs in [0.5,1,1.5]:   
        for l,accaz in enumerate(lambdarange):
            psi=measures[l,gs][2]
            psiresh=psi.reshape(lastm,2,lastm,2).transpose(1,3,0,2).reshape(4,-1)
            rhomagn=np.dot(psiresh,psiresh.conjugate().transpose())
            midrhodic[l,gs]=rhomagn
            
        rhos=[midrhodic[x,gs] for x in range(len(lambdarange))]
        plt.plot(lambdarange,eigvalsh(rhos))
        plt.show()