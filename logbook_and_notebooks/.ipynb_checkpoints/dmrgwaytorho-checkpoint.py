...
def DMRG_step(sys,site,env,m,H2):
    ...
    (energy,), psi0 = eigsh(superblock, k=1, which="SA")
    
    reshapedpsi=psi0.reshape(sys.basis_size,2,sys.basis_size,2).transpose(1,3,0,2).reshape(4,-1)
    
    rhomagn = np.dot(reshapedpsi, reshapedpsi.conjugate().transpose())
    ...
    return newblock, energy,rhomagn
...
def infinite_system_algorithm(block,site, L, m,H2):
    ...
    while 2 * block.length < L:
        ...
        block, energy , rhomagn = DMRG_step(block, site ,block, m,H2)
