...
def check_doppioni(a,b):
    if a!=b:
        if a in list(it.permutations(b)):
            return True
    else:
        return False
...
def get_dicts():
    ...
    for strings in dict_comb:
        ...
        lvalid_comb=list(valid_comb)

        final_dict[strings]=[list(it.chain(*i))for i in lvalid_comb if not check_doppioni(i ,lvalid_comb[0]) ]
...
#print(check_doppioni((("1","4"),("1","2")),(("1","2"),("1","4"))))
...
rhotr=rho_3new(dictr,permtr,gtr,10)
